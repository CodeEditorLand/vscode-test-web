...
suite("Workspace folder access", () => {
	...
	async function assertEntries(
		...
		const entrySorter = (
			e1: [string, vscode.FileType],
			e2: [string, vscode.FileType]
		) => {
			const d = e1[1] - e2[1];
			if (d === 0) {
		...
		let expected = expectedFolders
			.map<[string, vscode.FileType]>((name) => [
				name,
	...
	async function assertStats(
		...
		assert.deepStrictEqual(
			stats.type,
			isFile ? vscode.FileType.File : vscode.FileType.Directory
		);
	...
	test("File contents", async () => {
		...
		await assertContent(
			"/folder_with_utf_8_🧿/!#$%&'()+,-.0123456789;=@ABCDEFGHIJKLMNOPQRSTUVWXYZ[]^_`abcdefghijklmnopqrstuvwxyz{}~",
			"test_utf_8_🧿"
		);
	});
	...
	test("Create and delete directory", async () => {
		...
		await assertEntries(
			"/",
			["hello.txt", "world.txt"],
			["folder", "folder_with_utf_8_🧿", "more"]
		);
	...
	test("Rename", async () => {
		...
		await vscode.workspace.fs.rename(
			getUri("/folder/testing"),
			getUri("/folder/newTesting")
		);
