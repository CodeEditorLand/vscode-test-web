		...
		if (extension) {
			...
			for (const entry of entries) {
				if (entry.isDirectory() && entry.name.charAt(0) !== ".") {
					await processFolder(
						path.posix.join(relativePosixFolderPath, entry.name)
					);
				}
			}
		}
...
export async function getScannedBuiltinExtensions(
	...
	const extensionsUtil = await import(
		path.join(vsCodeDevLocation, "build", "lib", "extensions.js")
	);

	const localExtensions: IScannedBuiltinExtension[] =
