...
class Workbench {
	constructor(
		...
		private devCSSModules: string[],
		private readonly builtInExtensions: IScannedBuiltinExtension[] = [],
		private readonly productOverrides?: Record<string, any>,
	) {}

	async render(
		workbenchWebConfiguration: IWorkbenchOptions,
	): Promise<string> {
		if (this.productOverrides) {
			workbenchWebConfiguration.productConfiguration = {
				...workbenchWebConfiguration.productConfiguration,
		...
		try {
			const workbenchTemplate = await readFileInRepo(
				`views/workbench${this.esm ? "-esm" : ""}.html`,
			);
...
export default function (config: IConfig): Router.Middleware {
	...
	router.use(async (ctx, next) => {
		if (config.build.type === "sources") {
			...
			const productOverrides = await getProductOverrides(
				config.build.location,
			);
...
async function getProductOverrides(
	vsCodeDevLocation: string,
): Promise<Record<string, any> | undefined> {
	try {
		return JSON.parse(
			(
				await fs.readFile(
					path.join(vsCodeDevLocation, "product.overrides.json"),
				)
			).toString(),
		);
	} catch (e) {
...
async function isESM(vsCodeDevLocation: string): Promise<boolean> {
	try {
		const packageJSON = await fs.readFile(
			path.join(vsCodeDevLocation, "out", "package.json"),
		);
