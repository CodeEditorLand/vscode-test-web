...
async function getLatestVersion(
	...
	const update: DownloadInfo = await fetchJSON(
		`https://update.code.visualstudio.com/api/update/web-standalone/${quality}/latest`
	);
	return update;
}
...
async function downloadAndUntar(
	downloadUrl: string,
	destination: string,
	message: string
): Promise<void> {
...
export async function downloadAndUnzipVSCode(
	...
	const productName = `VS Code ${
		quality === "stable" ? "Stable" : "Insiders"
	}`;
