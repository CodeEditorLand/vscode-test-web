...
export default async function createApp(config: IConfig): Promise<Koa> {
		...
		cors({
			...
			origin: (ctx: Koa.Context) => {
				...
				if (
					...
					new RegExp(`^${ctx.protocol}://[^.]+\\.${ctx.host}$`).test(
						origin
					) // match subdomains of localhost
				) {
					return origin;
				}
	...
	app.use(async (ctx, next) => {
		...
		if (
			...
			ctx.response.body = `{"version":3,"file":"${basename(
				ctx.path
			)}","sections":[{"offset":{"line":2,"column":0},"map":${bodyContent} }]}`;
		}
	});
